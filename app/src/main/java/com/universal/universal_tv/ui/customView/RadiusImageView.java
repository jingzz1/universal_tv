...
public class RadiusImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public RadiusImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.RadiusImageView, defStyle, 0);

        final int index = a.getInt(R.styleable.RadiusImageView_android_scaleType, -1);
        if (index >= 0) {
            setScaleType(sScaleTypeArray[index]);
        }

        float radius = a.getDimension(R.styleable.RadiusImageView_riv_radius,0);

        mLeftTopCornerRadius = a.getDimension(
                R.styleable.RadiusImageView_riv_left_top_corner_radius, radius);
        mRightTopCornerRadius = a.getDimension(
                R.styleable.RadiusImageView_riv_right_top_corner_radius, radius);
        mLeftBottomCornerRadius = a.getDimension(
                R.styleable.RadiusImageView_riv_left_bottom_corner_radius, radius);
        mRightBottomCornerRadius = a.getDimension(
                R.styleable.RadiusImageView_riv_right_bottom_corner_radius, radius);
        ...
        mBorderWidth = a.getDimensionPixelSize(
                R.styleable.RadiusImageView_riv_border_width, 0);
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = a
                .getColorStateList(R.styleable.RadiusImageView_riv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = a.getBoolean(R.styleable.RadiusImageView_riv_oval, false);
        a.recycle();
    ...
    public void setRadiusDP(float radius){
        setCornerRadiiDP(radius,radius,radius,radius);
    }
